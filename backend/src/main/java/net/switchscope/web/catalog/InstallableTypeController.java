...
public class InstallableTypeController {
    ...
    public InstallableTypeTo update(@PathVariable UUID id, @RequestBody String jsonPayload) {
        ...
        InstallableTypeTo dto = objectMapper.readValue(jsonPayload, InstallableTypeTo.class);
        ...
        policyValidator.validate(InstallableTypeTo.class, presentFields, policy);

        // 3. Delegate to service for actual update (uses updateFromDto pattern)
        InstallableTypeEntity updated = service.updateFromDto(id, dto);
