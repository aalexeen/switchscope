...
Ветка `dev`, **не выложена: три коммита впереди `origin/dev`** (прежняя запись «совпадает с
локальной» устарела на `861ef6b`). Этот проход — Этап 3 раздела 1.1: фронтенд перестал решать по
роли и решает по правам. Первый коммит — `permissions` в ответах `/api/auth/login` и `/check`,
второй — сам гейт.

**Что закрыто:** Этап 1 раздела 1.1, **Этап 2 целиком — механизм, промотирование и глобальный
`ENFORCE`**, **Этап 3 — фронтенд на правах**, **раздел 1.2 (обнуление через PUT)** и **ПРИОРИТЕТ 2
целиком** — все 15 пунктов раздела. Приложение идёт в `ENFORCE` с пустым `enforce-domains`: список
промотированных доменов перестал быть конфигурацией и остался способом отката. **131 тест
зелёный** (было 127: четыре новых — на то, что личность несёт права).

**Что дальше по порядку:** три пункта, см. «Следующие шаги». Этап 4 раздела 1.1 — уборка (удалить
enum `Role`, явный `anyRequest()`); сведение контрактов сервисов и следом семь каталожных
контроллеров в общий базовый класс; пагинация с фильтрацией и сортировкой на `getAll` — сейчас
выдача не ограничена нигде. Там же записано, почему GraphQL рассмотрен и отклонён.
...
`USER × component.rack:read` — страница стоек перенаправляет на `/?error=unauthorized`; строки
возвращены — всё вернулось. Подробности, включая две ловушки (`/api/profile` стирал права из
хранилища; гейт маршрута отстаёт на одну навигацию) — `docs/frontend-permission-gate.md`.
...
Ровно по порядку. ПРИОРИТЕТ 1 закрыт целиком: Этап 2 вместе с промотированием всех пяти доменов и
глобальным `ENFORCE`, и раздел 1.2.

1. **Этап 4** раздела 1.1 — уборка: удалить enum `Role`, `hasRole` оставить только на
   `/api/admin/**`, дописать явный `anyRequest()`. Этап 3 закрыт. Ловушки, известные заранее:
   `ProfileController` отдаёт сущность `User` как есть, так что `roles` там обязаны остаться
   массивом строк (`services/auth.js` делает `roles.includes('ADMIN')`, и на этом же держится
   гейт `/users/**`); `UpdatePolicyResolver` завязан на enum и должен остаться ролевым — права
   на политику полей в модели нет; `anyRequest().denyAll()` — самый вероятный способ снова
   сломать springdoc, проверять живьём.

2. **Сведение контрактов сервисов, и следом — семь каталожных контроллеров в общий базовый
...
3. **Пагинация, фильтрация и сортировка на `getAll`.** Сейчас `AbstractCrudController.getAll()`
