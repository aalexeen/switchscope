...
**Что закрыто:** Этап 1 раздела 1.1 и **Этап 2 целиком, вместе с промотированием** — механизм
принуждения написан, покрыт тестами и работает; приложение идёт в `SHADOW` с промотированными
`catalog`, `location`, `installation`, `port`, `component`. Плюс два пункта ПРИОРИТЕТА 2
(логгер, `AccessDeniedException`).

**Что дальше по порядку:** раздел 1.2 (обнуление через PUT) → остаток ПРИОРИТЕТА 2 (11 пунктов) →
Этапы 3–4 раздела 1.1 (фронтенд, уборка). Глобальный `mode: ENFORCE` — отдельным шагом, см. ниже.
...
Ровно по порядку. Этап 2 закрыт целиком, вместе с промотированием всех пяти доменов.

1. **Раздел 1.2** — обнуление полей через PUT.
2. **ПРИОРИТЕТ 2**, 11 пунктов.
3. **Этапы 3–4** раздела 1.1 — фронтенд (`usePermissions()`, `meta.permission`, гейт кнопок) и
