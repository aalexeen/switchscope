...
тестами и работает; приложение идёт в `SHADOW` с промотированным доменом `catalog`. Плюс два
пункта ПРИОРИТЕТА 2 (логгер, `AccessDeniedException`).

**Что дальше по порядку:** остальные домены по одному → раздел 1.2 (обнуление через PUT) →
остаток ПРИОРИТЕТА 2 (11 пунктов) → Этапы 3–4 раздела 1.1 (фронтенд, уборка).

**Поведение API изменилось на каталожных маршрутах, и только на них.** `/api/catalogs/**` теперь
решает по `role_permissions`: `@PreAuthorize("hasRole('ADMIN')")` снята со всех каталожных
контроллеров, и USER, у которого есть `catalog.*`, снова может править каталоги — как и
предполагал фронтенд. Остальные домены по-прежнему в тени: решение считается и пишется в лог, но
не применяется. Живая проверка после промотирования (приложение на 8090, профиль `local`): USER
читает каталог 200, **USER создаёт запись 201 и удаляет её 204** (до промотирования было бы 403),
...
domains=[catalog]`, 109 эндпоинтов — 101 под правом, 8 вне модели, 0 без аннотации, 0
непроксируемых; 81 право, 0 отсутствующих в БД, 0 мёртвых строк.
...
- Режим принуждения — `switchscope.security.permission.mode` (`AUDIT` / `SHADOW` / `ENFORCE`,
  ...
  Сейчас там `[catalog]`. Одно следствие, которое легко упустить: с промотированным доменом
...
1. **Остальные домены по одному** (`component`, `location`, `installation`, …), в конце
   ...
   Приём тот же, что сработал на каталогах: добавить домен в `enforce-domains`, тем же коммитом
   снять всё, что дублирует проверку, прогнать тесты, поднять приложение и убедиться, что
   `PERMISSION-SHADOW-DENY` не появился. Откат — убрать домен из списка, без пересборки.
   Разница с каталогами в одном: у `component` / `location` / `installation` в `role_permissions`
   уже есть строки на оба ролей, но **старых `@PreAuthorize` там и не было** (в `AbstractCrudController`
   их не было никогда), так что промотирование этих доменов не открывает доступ, а закрывает —
   маршрут, который сейчас берёт любой залогиненный, начнёт требовать право.

   **`component` — не такой мелкий шаг, как звучит.** Домен — это то, что вернёт
   `PermissionCode.parse(code).domain()`: у бездотового кода это он сам целиком (`component:read`
   → `component`), у дотового — часть до первой точки (`component.rack:read` → тоже `component`).
   Так что `enforce-domains: [component]` включает разом и `ComponentController`, и все девять
   наследников `AbstractCrudController` (`rack`, `router`, `network-switch`, `patch-panel`,
   `connector`, `cable-run`, `access-point`). `location`, `installation` и `port` — по четыре
   бездотовых кода каждый, то есть по одному контроллеру. Начинать разумно с них.
