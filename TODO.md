...
**Что дальше по порядку:** Этапы 3–4 раздела 1.1 — фронтенд (`usePermissions()`, `meta.permission`,
гейт кнопок) и уборка (удалить enum `Role`, явный `anyRequest()`). Сведение контрактов сервисов —
отдельным шагом, см. ниже.
