...
Ветка `dev`, **не выложена: шесть коммитов впереди `origin/dev`** (прежняя запись «совпадает с
локальной» устарела на `861ef6b`). Этот проход закрыл **раздел 1.1 целиком** — Этапы 3 и 4, четыре
коммита: `permissions` в ответах `/api/auth/login` и `/check`, гейт во фронтенде, удаление enum
`Role` и поправка к утверждению про `denyAll()`.
