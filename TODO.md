...
`null`. Плюс два пункта ПРИОРИТЕТА 2 (логгер, `AccessDeniedException`). 81 тест зелёный.
...
422 `column manufacturer is NOT NULL`. Подробности и три отличия реализации от плана —
`docs/backend-partial-update.md`.
...
3. **Разметка `@FieldAccess` достроена** там, где её не было вовсе — `LocationTo`,
