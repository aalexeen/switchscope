...
1. **Остальные домены по одному** (`component`, `location`, `installation`, …), в конце
   ...
   уже есть строки на оба ролей, но **старых `@PreAuthorize` там и не было**, так что
   промотирование этих доменов не открывает доступ, а закрывает — маршрут, который сейчас берёт
   любой залогиненный, начнёт требовать право.
