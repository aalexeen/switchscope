...
явно. **137 тестов зелёных** (было 127: четыре на то, что личность несёт права, и шесть на
ёмкость стойки).
...
Ровно по порядку. ПРИОРИТЕТ 1 закрыт целиком: раздел 1.1 — все четыре этапа, от схемы прав до
удаления enum `Role`, — и раздел 1.2.

1. **Сведение контрактов сервисов, и следом — семь каталожных контроллеров в общий базовый
   класс.** Подготовлено, но не начато. Мешают четыре разных контракта: `DtoCrudService`
   возвращает `T`, `UpdatableCrudService` — `E` из одноимённого метода, плюс самодельные у
   `ComponentStatusService` и `LocationTypeService`. Второе препятствие раздел 1.2 уже убрал:
   PUT-обработчики стали одинаковыми с точностью до имени типа, копий `extractPresentFields`
   больше нет, и семь файлов теперь совпадают почти дословно. Осталась миграция сигнатур
   примерно по двадцати сервисам — работа механическая, но широкая, поэтому отдельным коммитом
   и не вперемешку с изменением поведения.

2. **Пагинация, фильтрация и сортировка на `getAll`.** Сейчас `AbstractCrudController.getAll()`
...
- **springdoc и сырые тела**: двадцать PUT и теперь девять POST принимают `@RequestBody String`,
