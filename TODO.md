...
Ветка `dev`. Второй проход по ПРИОРИТЕТУ 2 начинается с `7b2fc61` и заканчивается этим файлом:
шесть правок и два уточнения. Ветка **не выложена**: 31 коммит впереди `origin/dev`.

**Что закрыто:** Этап 1 раздела 1.1, **Этап 2 целиком вместе с промотированием**, **раздел 1.2
(обнуление через PUT)** и **ПРИОРИТЕТ 2 целиком** — все 15 пунктов раздела. Приложение
идёт в `SHADOW` с промотированными `catalog`, `location`, `installation`, `port`, `component`.
**124 теста зелёных** (было 83 до захода в ПРИОРИТЕТ 2; 115 после первых семи пунктов, минус шесть
случаев вместе с удалённым `AbstractCatalogControllerTest`, плюс пятнадцать новых).

**Что дальше по порядку:** Этапы 3–4 раздела 1.1 — фронтенд (`usePermissions()`, `meta.permission`,
гейт кнопок) и уборка (удалить enum `Role`, явный `anyRequest()`). Глобальный `mode: ENFORCE`
