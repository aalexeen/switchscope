...
**Что закрыто:** Этап 1 раздела 1.1 и **Этап 2 целиком** — механизм принуждения написан, покрыт
тестами и работает; приложение идёт в `SHADOW` с промотированными `catalog`, `location`,
`installation`, `port`. Плюс два пункта ПРИОРИТЕТА 2 (логгер, `AccessDeniedException`).

**Что дальше по порядку:** промотировать `component` (последний домен) → раздел 1.2 (обнуление
через PUT) → остаток ПРИОРИТЕТА 2 (11 пунктов) → Этапы 3–4 раздела 1.1 (фронтенд, уборка).

**Поведение API решается по `role_permissions` везде, кроме домена `component`.** Из четырёх
промотированных доменов только `catalog` при этом сменил доступ: `@PreAuthorize("hasRole('ADMIN')")`
снята со всех каталожных контроллеров, и USER снова может править справочники — как и предполагал
фронтенд. У `location` / `installation` / `port` старой аннотации не было, и обе роли держат все
их права, так что там промотирование не изменило ни одного ответа — только сделало проверку
настоящей. `component` пока в тени: решение считается, пишется в лог и не применяется.

Живая проверка после промотирования (приложение на 8090, профиль `local`): USER читает каталог
200, **создаёт запись 201 и удаляет её 204** (до промотирования обе были бы 403); USER читает
`/api/locations`, `/api/installations`, `/api/ports`, `/api/components` — 200; `DELETE` на
несуществующий id в первых трёх — **404, а не 403** (право пропустило, запрос дошёл до сервиса);
ADMIN читает матрицу 200, USER матрицу не читает 403, аноним 401; **ни одного
`PERMISSION-SHADOW-DENY` в логе**. Отчёт реестра на старте: `mode=SHADOW, enforced
domains=[catalog, location, installation, port]`, 109 эндпоинтов — 101 под правом, 8 вне модели,
0 без аннотации, 0 непроксируемых; 81 право, 0 отсутствующих в БД, 0 мёртвых строк.
...
- Режим принуждения — `switchscope.security.permission.mode` (`AUDIT` / `SHADOW` / `ENFORCE`,
  ...
  Сейчас там `[catalog, location, installation, port]` — не промотирован только `component`.
...
Ровно по порядку. Этап 2 закрыт целиком; первый пункт продолжает его тем же приёмом.

1. **Промотировать `component`** — последний домен. `enforce-domains` сейчас
   `[catalog, location, installation, port]`; после него остаётся заменить весь список на
   `mode: ENFORCE` целиком. `hasRole` остаётся только на `/api/admin/**` — и остаётся он там
   правилом в `SecurityConfig` (`requestMatchers("/api/admin/**").hasRole(...)`), а не аннотацией:
   после промотирования `catalog` в контроллерах не осталось ни одной `@PreAuthorize`.
   Приём отработан: добавить домен в `enforce-domains`, тем же коммитом снять всё, что дублирует
   проверку (для `component` — ничего, там `@PreAuthorize` не было), прогнать тесты, поднять
   приложение и убедиться, что `PERMISSION-SHADOW-DENY` не появился. Откат — убрать домен из
   списка, без пересборки.

   **`component` — не такой мелкий шаг, как звучит,** и потому оставлен напоследок. Домен — это
   то, что вернёт `PermissionCode.parse(code).domain()`: у бездотового кода это он сам целиком
   (`component:read` → `component`), у дотового — часть до первой точки (`component.rack:read` →
   тоже `component`). Так что `enforce-domains: [component]` включает разом и `ComponentController`,
   и все девять наследников `AbstractCrudController` (`rack`, `router`, `network-switch`,
   `patch-panel`, `connector`, `cable-run`, `access-point`) — 36 прав из 81. Это ровно те маршруты,
   на которых живут bridge-методы, так что перед промотированием стоит перечитать
   `PermissionEnforcementTest`.

   **Что промотирование этих доменов меняет и чего не меняет.** У `location` / `installation` /
   `port` / `component` старых `@PreAuthorize` не было никогда, так что промотирование не
   открывает доступ, а закрывает: маршрут, который брал любой залогиненный, начинает требовать
   право. Но **сегодня ни один вызов от этого не отказывается** — и USER, и ADMIN держат все
   права этих доменов (единственное расхождение ролей во всей таблице — `system`, только у ADMIN).
   То есть эффект чисто структурный: решение становится настоящим, а не вычисленным и выброшенным.
   Живая проверка сводится к «ничего не сломалось»: `DELETE /api/locations/<несуществующий>` под
   USER должен дать **404, а не 403** — 404 означает, что право пропустило и запрос дошёл до
   сервиса.
2. **Раздел 1.2** — обнуление полей через PUT.
3. **ПРИОРИТЕТ 2**, 11 пунктов.
4. **Этапы 3–4** раздела 1.1 — фронтенд (`usePermissions()`, `meta.permission`, гейт кнопок) и
...
**Что осталось:** промотировать `component` — последний непромотированный домен, и с ним
`enforce-domains` можно заменить на `mode: ENFORCE` целиком.
