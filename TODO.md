...
пишется — он протухает быстрее всего остального). Состояние кода — **`7c4b4c5`**; всё, что позже, правит только этот раздел.

**Если контекст только что очищен — читать так:** очередь работ кончилась, в ней остался один
пункт (пагинация, «Следующие шаги»), и один вопрос ждёт решения пользователя — общая правка под
`POST /api/ports` (см. «мелочи» там же). Всё остальное в этом разделе — что и почему уже сделано;
самое свежее идёт первым, старое — ниже, начиная с «Что закрыто».
...
**Что дальше:** один пункт, см. «Следующие шаги» — пагинация с фильтрацией и сортировкой на
`getAll`, сейчас выдача не ограничена нигде (`Pageable` в `src/main/java` не встречается ни разу).
Там же записано, почему GraphQL рассмотрен и отклонён, и там же «мелочи», из которых одна ждёт
решения пользователя, а не работы.
...
Остался один пункт. ПРИОРИТЕТ 1 закрыт целиком: раздел 1.1 — все четыре этапа, от схемы прав до
удаления enum `Role`, — и раздел 1.2. Сведение контрактов и схлопывание каталожных контроллеров
закрыто 2026-08-24 двумя коммитами — см. «Точку входа».

1. **Пагинация, фильтрация и сортировка на `getAll`.** Сейчас `AbstractCrudController.getAll()`
   отдаёт `List<T>` целиком, и `Pageable` не встречается в `src/main/java` ни разу — то есть
   ни одного маршрута с ограничением выдачи во всём API. Фронт это уже чувствует по-своему:
   composables тянут таблицу целиком в синглтон, в том числе `useComponents` под выпадашку
   «родительский компонент». Это единственное место, которое масштабируется плохо на самом деле,
   и оно не про число эндпоинтов. Порядок работ, когда дойдут руки: `Pageable` в
   `AbstractCrudController.getAll` (с обратной совместимостью — без параметров отдавать как
   прежде), затем фильтр по полям DTO, затем сортировка; фронт — постранично в таблицах и
   поиск по серверу в `searchable-select` вместо загрузки всего справочника.
