...
Ветка `dev`. Последний коммит — `7bf73cc` (валидация тел, которые Spring больше не связывает).
Ветка **не выложена**: 29 коммитов впереди `origin/dev` (обновление этого файла — 30-й).
...
**123 теста зелёных** (было 83 до захода в ПРИОРИТЕТ 2; 115 после первых семи пунктов, минус шесть
случаев вместе с удалённым `AbstractCatalogControllerTest`, плюс четырнадцать новых).
