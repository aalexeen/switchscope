...
явно. **131 тест зелёный** (было 127: четыре новых — на то, что личность несёт права).
...
- **`Rack` падает на пустом `rackUnitsTotal`**: колонка `rack_units_total` nullable, а вычисляемые
  геттеры разыменовывают её — `POST /api/housing/racks` без этого поля отвечает 500 `NullPointerException`.
  Дефект старый, не связан с сырым телом (с типизированным `@RequestBody` было то же самое);
