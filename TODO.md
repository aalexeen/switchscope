...
Ветка `dev`. Последние коммиты: `285263d` (промотирован последний домен прав),
`ca77367` и `155ba64` (раздел 1.2 — обнуление полей через PUT). Ветка **не выложена**:
13 коммитов впереди `origin/dev`.

**Что закрыто:** Этап 1 раздела 1.1, **Этап 2 целиком вместе с промотированием** и **раздел 1.2
(обнуление через PUT)**. Приложение идёт в `SHADOW` с промотированными `catalog`, `location`,
`installation`, `port`, `component`; PUT на всех маршрутах различает отсутствующее поле и явный
`null`. Плюс два пункта ПРИОРИТЕТА 2 (логгер, `AccessDeniedException`). 83 теста зелёные.

**Что дальше по порядку:** остаток ПРИОРИТЕТА 2 (12 пунктов) → Этапы 3–4 раздела 1.1 (фронтенд,
уборка). Глобальный `mode: ENFORCE` и сведение контрактов сервисов — отдельными шагами, см. ниже.
