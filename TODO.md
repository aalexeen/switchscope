...
(обнуление через PUT)** и **семь пунктов ПРИОРИТЕТА 2 в этом проходе** (плюс два, закрытых
раньше; в разделе теперь 15 пунктов, из них 9 закрыты). Приложение
идёт в `SHADOW` с промотированными `catalog`, `location`, `installation`, `port`, `component`.
**115 тестов зелёных** (было 83 до захода в ПРИОРИТЕТ 2).

**Что дальше по порядку:** остаток ПРИОРИТЕТА 2 — **шесть пунктов**: пять старых и один новый
(список ниже, незакрытые помечены `[ ]`) → Этапы 3–4 раздела 1.1 (фронтенд, уборка). Глобальный `mode: ENFORCE`
...
1. **ПРИОРИТЕТ 2**, осталось шесть пунктов: bulk-delete мимо cascade, `clear-checksums`
...
свои находки, ещё одна нашлась при сверке `@Size`. Всего пунктов пятнадцать; после прохода
2026-08-23 закрыты девять, **осталось шесть**:
