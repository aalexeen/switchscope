...
Ветка `dev`, и она выложена: `origin/dev` совпадает с локальной. Прежняя запись здесь — «не
выложена, 31 коммит впереди» — устарела. Этот проход — один коммит: глобальный `mode: ENFORCE`,
опустошённый `enforce-domains` и два дефекта советника, которых `SHADOW` не показывал.

**Что закрыто:** Этап 1 раздела 1.1, **Этап 2 целиком — механизм, промотирование и глобальный
`ENFORCE`**, **раздел 1.2 (обнуление через PUT)** и **ПРИОРИТЕТ 2 целиком** — все 15 пунктов
раздела. Приложение идёт в `ENFORCE` с пустым `enforce-domains`: список промотированных доменов
перестал быть конфигурацией и остался способом отката. **127 тестов зелёных** (было 124 до этого
прохода: два новых случая воспроизводят вскрытые дефекты, третий проверяет, что суженный pointcut
достаёт до каждого эндпоинта).

**Что дальше по порядку:** три пункта, см. «Следующие шаги». Этапы 3–4 раздела 1.1 — фронтенд
(`usePermissions()`, `meta.permission`, гейт кнопок) и уборка (удалить enum `Role`, явный
`anyRequest()`); сведение контрактов сервисов и следом семь каталожных контроллеров в общий
базовый класс; пагинация с фильтрацией и сортировкой на `getAll` — сейчас выдача не ограничена
нигде. Там же записано, почему GraphQL рассмотрен и отклонён.
...
1. **Этапы 3–4** раздела 1.1 — фронтенд (`usePermissions()`, `meta.permission`, гейт кнопок) и
