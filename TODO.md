...
Ветка `dev`, **не выложена: первый невыложенный коммит — `861ef6b`** (прежняя запись «совпадает с
локальной» устарела на нём же; счёт коммитов здесь не пишется — он протухает быстрее всего
остального). Этот проход закрыл **раздел 1.1 целиком** — Этапы 3 и 4: `permissions` в ответах
`/api/auth/login` и `/check`, гейт во фронтенде, удаление enum `Role`, затем поправки к двум
собственным утверждениям — про `denyAll()` и про то, чего не умеет проверялка кодов.
...
**Что дальше по порядку:** два пункта, см. «Следующие шаги». Сведение контрактов сервисов и следом
семь каталожных контроллеров в общий базовый класс; пагинация с фильтрацией и сортировкой на
`getAll` — сейчас выдача не ограничена нигде. Там же записано, почему GraphQL рассмотрен и
отклонён.
