  ...
  <section class="bg-blue-50 px-4 py-10">
    <div class="container-xl lg:container m-auto">
      ...
      <div v-if="error" class="bg-white rounded-lg shadow p-6 text-center text-red-500">
        <p class="mb-4">Error loading component models. Please try again.</p>
        <button
          @click="fetchComponentModels(true)"
      ...
      <div v-else-if="isLoading" class="bg-white rounded-lg shadow p-8 text-center text-gray-500">
        <PulseLoader />
        <p class="mt-4">Loading component models...</p>
      </div>

      <!-- Show component model listing table when done loading -->
      <div v-else-if="displayedModels.length > 0" class="bg-white rounded-lg shadow overflow-hidden">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" title="Verified">
                  Verified
                </th>
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Description
