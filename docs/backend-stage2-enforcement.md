...
> **Статус: реализовано целиком**, включая промотирование и глобальный `ENFORCE`. Всё, что ниже
> помечено как гипотеза или предположение, к моменту реализации проверено — см. последний раздел.
> Приложение идёт в режиме `ENFORCE` с пустым `enforce-domains`; 126 тестов зелёные. Что дало
> каждое промотирование — в разделе «Промотирование: чем кончилось»; что дал перевод самого
> режима и какие два дефекта он вскрыл — в разделе «Глобальный `ENFORCE`».
>
...
## Глобальный `ENFORCE`: два дефекта, которые прятал `SHADOW`

Правка в `application.yaml` на две строки: `mode: SHADOW` → `ENFORCE`, `enforce-domains` — в пусто.
Список опустошён не ради опрятности. Домен из списка принуждается **независимо** от режима, так что
оставленные в нём пять доменов сделали бы `mode: SHADOW` неправдой: вернув режим ради наблюдения,
читатель получил бы всё те же отказы в пяти доменах из шести. Список остаётся способом отката
(`SHADOW` плюс перечисление того, что должно продолжать отказывать), а не конфигурацией.

Что перевод режима меняет — ровно три вещи:

1. `system` — одно право на `/api/admin/**` — начинает проверяться по-настоящему;
2. эндпоинт **без** кода права начинает отказывать, а не писать строку в лог (таких ноль);
3. старт падает на любой ошибке реестра, а не только на ошибке внутри промотированного домена.

Ожидание было: ответы не изменятся ни на одном маршруте, потому что пять доменов промотированы
поимённо, а обе роли держат все их права. Изменились — на четырёх springdoc-овских маршрутах и на
всех `POST`/`PUT` профиля. Оба дефекта жили и в `SHADOW`, просто писали `PERMISSION-SHADOW-DENY` в
лог вместо того, чтобы отказывать.

**Дефект 1: pointcut шире приложения.** Ресурсы springdoc — такие же `@RequestMapping`-обработчики,
и советник доставал до них; скан же их пропускает намеренно (`OWN_PACKAGE`, «not ours to gate»).
Значит, реестр о них ничего не знает, а «ничего не знает» — это `Unknown`, а `Unknown` закрывает.
Под `ENFORCE` это дало 403 на `/v3/api-docs`, `/v3/api-docs/swagger-config` и Swagger UI на `/` —
все три `permitAll` в цепочке фильтров и отказаны **после** неё. В логе — `Refusing
OpenApiWebMvcResource#openapiJson`, `SwaggerConfigResource#openapiJson`,
`SwaggerWelcomeWebMvc#redirectToUi`.

**Дефект 2: pointcut шире эндпоинтов.** Вторая половина union-а матчила **класс** с
`@RequestMapping`, то есть все методы контроллера, а не только обработчики. У
`AbstractUserController` есть `@InitBinder initBinder`, который регистрирует
`UniqueMailValidator` и вызывается при разборе тела — до того, как советник сработал бы на самом
обработчике. Реестр о нём, разумеется, не знает: в handler mapping его нет. Итог — `Unknown`,
отказ, и `POST`/`PUT` на `/api/profile` отвечали 403 ещё на разборе аргументов, тогда как `GET`,
которому байндер не нужен, проходил. Шесть тестов (`ProfileUpdateEndpointTest`,
`RegistrationEndpointTest`) увидели это первыми: `Status expected:<204> but was:<403>`.

Общее у обоих — правило «`Unknown` закрывает» верно **для эндпоинтов**, а под него попало то, что
эндпоинтом не является. Pointcut сужен с двух сторон:
...
`isOwnEndpoint` — тот же самый тест, которым скан отсеивает чужие бины (одна константа на обе
стороны, чтобы «не сканируется» и «не принуждается» не разъехались). `HANDLER_METHODS` спрашивает
маппинг так же, как его ищет `RequestMappingHandlerMapping`: `AnnotationUtils.findAnnotation` по
иерархии метода, поэтому `@GetMapping` как мета-аннотация, переопределение без повторной аннотации
и пять маппингов, объявленных в `AbstractCrudController` на девять наследников, — матчатся, а
`@InitBinder` и подобное — нет.

**Сужение открывает свою дыру, и она закрыта тестом.** Эндпоинт, до которого pointcut не достаёт,
не проверяется вообще — это та же немота, что у непроксируемого эндпоинта, только с другой
стороны: 200 там, где право говорило 403. `PermissionCoverageTest.everyEndpointIsReachedByTheAdvisor`
берёт pointcut из самого бина-советника и прогоняет по нему все обработчики из handler mapping,
предварительно сверив их число с числом в отчёте (иначе тест мог бы «пройти», не проверив ничего).
Что тест **находит**, а не просто молчит, проверено подменой матчера на наивный
(`isAnnotationPresent`, без разрешения мета-аннотаций): он перечислил недостижимые эндпоинты
поимённо (`ComponentStatusController#get`, `LocationController#getAll`, …). Оба дефекта, кроме
того, воспроизведены тестом **до** правки — `foreignHandlerIsNotRefused` и
`nonHandlerMethodIsNotRefused` в `PermissionEnforcementTest` падали с 403.

**Живая проверка** (профиль `local`, порт 8090):

- отчёт реестра: `mode=ENFORCE, enforced domains=[]`, 109 эндпоинтов — 101 под правом, 8 вне
  модели, 0 без аннотации, 0 непроксируемых; 81 право, 0 отсутствующих в БД, 0 мёртвых строк.
  Сам факт, что старт прошёл, теперь значит больше: под `ENFORCE` любая из этих ошибок его роняет;
- аноним: `/v3/api-docs`, `/v3/api-docs/swagger-config`, `/swagger-ui/index.html` — 200, `/` — 302,
  `POST /api/profile` с телом `{}` — 422 (то есть запрос дошёл до валидации), `/api/components` — 401;
- USER: `/api/components`, `/api/catalogs/component-types`, `/api/housing/racks` — 200;
  `DELETE` несуществующей стойки — 404, а не 403; `PUT /api/profile` теми же значениями — 204;
  матрица — 403 (её закрывает `hasRole` в `SecurityConfig`, а не советник);
- ADMIN: матрица — 200;
- ни одной строки `Refusing` в логе. Нулевое число `PERMISSION-SHADOW-DENY` больше ничего не
  доказывает — в глобальном `ENFORCE` эта ветка недостижима, так что искать нужно `Refusing`.

**`system` действительно проверяется — проверено удалением строки.** Одинаковых 200 мало: `/api/admin/**`
и так закрыт по роли, поэтому «ADMIN читает матрицу» согласуется и с рабочей проверкой, и с
выключенной. Из `role_permissions` убрана строка `Administrator × system.permission:read`: та же
матрица под ADMIN сменила 200 на **403** после протухания кэша (TTL 60 с), а после возврата строки
вернулась к 200. Строка на месте.
...
## Этап 4 — уборка: enum `Role` удалён

Права стали единственной моделью доступа ещё в Этапе 2, но роль оставалась в двух видах сразу:
строка в `roles` и константа enum-а, который был одновременно `GrantedAuthority` и значением
колонки `user_role.role`. Этап 4 оставил один вид.

**Что изменилось в коде.** `User.roles` — `Set<RoleEntity>` через `@ManyToMany` на
`user_role(user_id, role_id)`, EAGER, как и было (аутентификация stateless, роли нужны на каждый
запрос). `AuthUser.hasRole` принимает код, а не константу. `SecurityConfig` собирает authorities
сам: `ROLE_` + код роли плюс её права, и префикс теперь написан **один раз** — `AuthUser.ROLE_PREFIX`,
которым же `permissions()` отличает одно от другого. `hasRole(Role.ADMIN.name())` стал
`hasRole(RoleEntity.ADMIN_CODE)`; кодов-констант ровно два, `ADMIN_CODE` и `USER_CODE`, и третья
роль не потребует ни одной. `UpdatePolicyResolver` остался ролевым — права на политику полей в
модели нет, и придумывать его в уборочном этапе неправильно.

**Роль при регистрации.** `UserMapper` больше не может выдать роль выражением `{Role.USER}` —
роль это строка в таблице. Выдаёт `UserService.create`, читая её по коду из `roles`: пустой набор
означает «не сказано», а не «без ролей». Из запроса роль по-прежнему не приходит.

**Форма на проводе не изменилась, и это главное ограничение этапа.** `/api/profile` отдаёт саму
сущность `User`, а `services/auth.js` делает `roles.includes('ADMIN')` — и на этом же держится
ролевой гейт `/users/**` во фронтенде. Поэтому `roles` сериализуется списком кодов
(`@JsonSerialize(contentConverter = ...)` на поле): одно логическое свойство, а не поле плюс
дублирующий геттер, который пришлось бы держать в согласии с ним. `LoginResponseTo.roles` по той
же причине `Set<String>`.

**Миграция** — `init/07-user-role-drop-role.yaml`, включённый **после** `fill/06`: он делает
`role_id` NOT NULL, добавляет `uk_user_role_role_id` на `(user_id, role_id)`, снимает `uk_user_role`
и роняет колонку `role`. Перед этим — `UPDATE` для строк, которые приложение вставило после
`fill/06` и у которых `role_id` пуст (через `@ElementCollection` писалась только строка). Порядок
включений важен: на чистой базе `fill/02` ещё вставляет старую колонку (в мастере это строки 129,
141 и 149 — порядок проверен чтением, прогоном на чистой базе нет: у миграционной учётки нет права
`CREATEDB`).

**Rollback прогнан, а не только написан.** `./mvnw liquibase:rollback -Dliquibase.rollbackCount=1`
(в плагине 4.30 это цель `rollback`, а не `rollbackCount`) вернул колонку `role` — NOT NULL,
значения восстановлены из `role_id`: ADMIN 1, USER 3, — `uk_user_role` на месте, `role_id` снова
nullable. Обратный `liquibase:update` вернул схему к `role_id NOT NULL` + `uk_user_role_role_id`,
все четыре строки целы.

**`anyRequest().denyAll()`** дописан — не потому, что там была дыра, а чтобы решение про путь,
о котором никто не подумал, было записано в файле. Замерено после: `/whatever` — 401 анониму, 403
аутентифицированному; springdoc цел (`/v3/api-docs` и `swagger-config` — 200, `/swagger-ui/index.html`
— 200, `/` — 302); ошибочный диспатч цел (`GET` несуществующей стойки — 404, а не 403); статических
ресурсов у бэкенда нет вовсе. Что поведение при этом **не изменилось**, подтверждается самим
артефактом: в `spring-security-web-6.5.3` класс `RequestMatcherDelegatingAuthorizationManager`
несёт строку `Denying request since did not find matching RequestMatcher`, то есть несовпавший
запрос отклонялся и без `anyRequest()`. Запуском старого кода это не перепроверялось — он не
поднимается на мигрированной базе, — так что «те же коды ответа до и после» строго говоря
не замерялись; отклонение как таковое подтверждено.

**Чем проверено.** 131 тест зелёный **на `./mvnw clean test`** — не на инкрементальной сборке:
после удаления класса в `target/classes` остаётся его `.class`, и зелёный прогон без `clean`
не доказывал бы, что enum больше никому не нужен. Живьём на `local`: отчёт реестра тот же (109 эндпоинтов, 101
под правом, 8 вне модели, ноль находок); `/api/auth/check` под ADMIN — `roles: ["ADMIN","USER"]`,
81 право; `/api/profile` — `roles: ["USER","ADMIN"]`, то есть форма прежняя; матрица под ADMIN 200,
под USER 403, анониму 401 — ролевой рубеж на месте; регистрация нового пользователя через
`POST /api/profile` дала 201 и `roles: ["USER"]`, новая учётка аутентифицировалась с 80 правами и
удалила себя (204). Во фронтенде под USER страница стоек показывает те же четыре строки и три
кнопки, под ADMIN открывается `/users/**` — то есть обе половины, права и роль, переживают удаление
enum-а.
