...
ресурсов у бэкенда нет вовсе. **Чем это было до правки — не замерено:** старый код не запускается
на мигрированной базе, а замер потребовал бы откатить и changeset, так что утверждение плана
«Spring Security 6.5 отклоняет несовпавший запрос сам» осталось непроверенным. Прежнее поведение
на несовпавшем пути могло быть и 404.
