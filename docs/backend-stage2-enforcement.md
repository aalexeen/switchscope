...
> **Статус: реализовано**, кроме последнего шага (промотировать `catalog` в `ENFORCE` и снять
> восемь `@PreAuthorize`). Всё, что ниже помечено как гипотеза или предположение, к моменту
> реализации проверено — см. последний раздел. Приложение идёт в режиме `SHADOW`; 56 тестов зелёные.
>
...
- сам переход в `ENFORCE` для `catalog`: это последний шаг и он ещё не сделан.
