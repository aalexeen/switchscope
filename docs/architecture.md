# SwitchScope — Developer Guide

Architecture, conventions and day-to-day commands for this repository.
...
**Backend**: Java 21, Spring Boot 3.5.5, Hibernate 7.2, PostgreSQL 18, Liquibase 4.30, MapStruct 1.5.5, Lombok, SNMP4J 3.9.6, SSHJ 0.40.0, Testcontainers 1.19.3
...
- Local PG18 cluster listens on port **5433** (PG16 occupies the default 5432 — do not use it)
- Database `switchscope`, schema `switchscope`, user `switchscope_user_back`
- JDBC: `jdbc:postgresql://localhost:5433/switchscope`
- Credentials live in gitignored files: `.local.props`, `backend/src/main/resources/config.properties`,
  `backend/src/main/resources/application-local.yaml`, `backend/.mvn/maven.config`, `.mcp.json`
- `backend/.mvn/maven.config` is what feeds `database.url` to the liquibase-maven-plugin
- Bootstrap a fresh DB: `./mvnw liquibase:update` recreates schema + seed data from the changelog
